...
from password_manager.view.wins.add_new_app import addNewApp
...
class mainWin(tk.Tk):
    ...
    def __init__(self):
        ...
        # btns---------------------------------------------------------------------------
        self.check_btn = tk.Button(self.btns_frame, 
                                   text=conf['view_btn_caption'], 
                                   width=20, height=2, 
                                   command=self.__check_btn_click
                                   )
        self.check_btn.grid(row=1, pady=5)

        self.add_btn = tk.Button(self.btns_frame, 
                                 text=conf['add_btn_caption'], 
                                 width=20, height=2, 
                                 command=self.__add_btn_click
                                 )
        self.add_btn.grid(row=2, pady=5)

        self.change_btn = tk.Button(self.btns_frame, 
                                    text=conf['change_btn_caption'], 
                                    width=20, height=2,
                                    state='disabled'
                                    )
        self.change_btn.grid(row=3, pady=5)

        self.del_btn = tk.Button(self.btns_frame, 
                                 text=conf['del_btn_caption'], 
                                 width=20, height=2,
                                 state='disabled'
                                 )
    ...
    def __check_btn_click(self):
        ...
        check_app_data_win = checAppDatakWin(data, self)
        check_app_data_win.grab_set()

    
    def __add_btn_click(self):
        add_new_app_win = addNewApp()
        add_new_app_win.grab_set()
